...
`listen`, `log.format` and `log.save_to_file` are the exceptions — the port is
already bound and the log handler is already built, so changing those needs a
restart, and a reload that tries will say so.
...
save_to_file = false    # also write logs/auth-hub-YYYY-MM-DD.log
```
...
`save_to_file = true` also appends every line to `logs/auth-hub-YYYY-MM-DD.log`,
next to the binary:

```
logs/
  auth-hub-2026-07-14.log
  auth-hub-2026-07-15.log
```

The date in the name is the rotation — a new file each day, picked up as the day
turns, so nothing to configure and no logrotate to point at it. Old days are
just files: delete them when you're bored of them. The console keeps everything
either way, so `docker compose logs` doesn't go quiet when you turn this on.

In Docker the working directory is `/`, so the files land in `/logs` inside the
container and go when the container does. Mount a volume there to keep them:
